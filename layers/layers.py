...
def print_result(current_epoch, total_epoch, current_iters, total_iters, begin, loss, lr, round):
    list1 = list('[                    ]')
    list2 = list('[                    ]')
...
class Train:
    ...
    def train(self, model, optimizer, train_questions, train_answer, test_questions, test_answer, batch_size, max_epoch,
              word_id, log_dir=None, Tensorboard_reloadInterval=30, log_file_name='', Enale_Tensorboard=True):
        max_iter = len(train_questions) // batch_size
        loss_cumulate = 0
        begin = time.time()
